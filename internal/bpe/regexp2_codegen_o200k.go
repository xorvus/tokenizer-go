...
func O200KPattern_FindFirstChar(r *regexp2.Runner) bool {
	pos := r.Runtextpos
...
func O200KPattern_Execute(r *regexp2.Runner) error {
	atomic_stackpos := 0
...
LoopBody:
	;
	if r.StackDepth() > 99999 {
		return regexp2.ErrBacktrackingStackLimit
	}
	r.StackPush(pos)
...
AlternationBranch2:
	...
	iteration5 = 0
	for iteration5 < len(slice) && ((slice[iteration5] < 128 && helpers.IsInASCIIBitmap(slice[iteration5], 0xfc00fffeffffc1ff, 0x78000001f8000001)) || (slice[iteration5] >= 128 && set_93b362dc942d41f83c0905ca6229c31c41f863d4ab4d2d9e17dae21b5b922113.CharIn(slice[iteration5]))) {
		iteration5++
	}
	...
	iteration6 = helpers.IndexOfAnyExcept2(slice, '\n', '\r')
	if iteration6 < 0 {
...
func init() {
	regexp2.RegisterEngine("[^\\r\\n\\p{L}\\p{N}]?[\\p{Lu}\\p{Lt}\\p{Lm}\\p{Lo}\\p{M}]*[\\p{Ll}\\p{Lm}\\p{Lo}\\p{M}]+(?i:'s|'t|'re|'ve|'m|'ll|'d)?|[^\\r\\n\\p{L}\\p{N}]?[\\p{Lu}\\p{Lt}\\p{Lm}\\p{Lo}\\p{M}]+[\\p{Ll}\\p{Lm}\\p{Lo}\\p{M}]*(?i:'s|'t|'re|'ve|'m|'ll|'d)?|\\p{N}{1,3}| ?[^\\s\\p{L}\\p{N}]+[\\r\\n]*|\\s*[\\r\\n]+|\\s+(?!\\S)|\\s+", regexp2.RuntimeEngineData{
		...
		CapsList:      nil,
		CapSize:       1,
		FindFirstChar: O200KPattern_FindFirstChar,
		Execute:       O200KPattern_Execute,
	}, regexp2.None)
	var _ = helpers.Min
